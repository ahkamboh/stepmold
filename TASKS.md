...
- [x] **T1 — Model protocol + FakeModel**
  `stepmold/model.py`. Define `Model` protocol: `generate(prompt: str, grammar: dict | None, max_tokens: int) -> str`.
...
- [x] **T2 — Pack format: load + validate**
  `stepmold/pack.py`. A StepmoldPack is a directory: `manifest.yaml` (name, version, entry node, model hint),
...
- [x] **T3 — Schema/grammar layer**
  `stepmold/grammar.py`. `schema_to_grammar(schema: dict) -> dict` (pass-through struct for now, backend
...
- [x] **T4 — Graph walker**
  `stepmold/graph.py`. Execute nodes from entry, following edges. Node types: `generate`, `assert`, `end`.
...
- [x] **T5 — Two-stage codegen (think → emit)**
  `stepmold/codegen.py`. For nodes with `two_stage: true`: first an unconstrained `think` generation
...
- [x] **T6 — Verify-before-commit + retry ladder**
  `stepmold/verify.py`, wired into the walker. Output must pass (a) grammar validation, (b) optional
...
- [x] **T7 — Checkpointing**
  `stepmold/state.py`. Persist state after each committed node, keyed by run id. Backend: stdlib
...
- [x] **T8 — Evalset runner**
  `stepmold/eval.py`. Load `evalset.jsonl` (`{"input": {...}, "expect": {...}}`), run each case through
...
- [x] **T9 — CLI**
  `stepmold/cli.py` + `python3 -m stepmold`. Commands: `stepmold run <pack> --input '<json>'`,
  `stepmold eval <pack>`, `stepmold validate <pack>`. Stdlib `argparse` only. Exit code 1 on eval failure.
...
- [x] **T10 — Example pack: support_triage**
  ...
  `stepmold eval examples/support_triage` passes end to end in CI.
...
- [x] **T11 — Real backend adapter (CODE ONLY, DO NOT RUN)**
  `stepmold/backends/openai_compat.py`. `OpenAICompatModel(base_url, model, api_key=None)` speaking the
