...
A stepmold pack is scoreable with no GPU, no network and no API key. Every example pack in
this repo ships a scripted model inside itself, so `stepmold eval` is a CI gate that needs
nothing but Python:

```
$ python3 -m stepmold eval examples/incident_triage
incident_triage: 13/13 cases passed
```

That works because stepmold never calls a model directly — it calls anything satisfying the
`Model` protocol (`stepmold/model.py:Model`), and `FakeModel` is a drop-in. This page
...
| Limit | What it means |
| --- | --- |
| Scoring is **exact equality only** | No substring, no tolerance, no set-equality. `["a","b"] != ["b","a"]`. See `stepmold/eval.py:_compare`. |
| **Nothing validates the fake script against the evalset** | A missing key is not a load error, and neither is a script file that does not exist. `stepmold validate` exits 0 on both. It surfaces mid-eval as a per-case failure, or as a raw traceback under `stepmold run`. |
...
```
$ python3 -m stepmold validate ticket_triage
ticket_triage v1: 5 nodes, 3 edges, 3 evalset cases, entry 'classify'

$ python3 -m stepmold eval ticket_triage
...
```
$ python3 -m stepmold run ticket_triage --input '{"ticket_id": "T-2", "text": "how do I export my invoices to csv?"}'
...
(`stepmold/cli.py:_fake_model`). It must be a non-empty JSON **array** or JSON **object**.

Two rough edges at that boundary, both real:

```
$ printf '{}' > ticket_triage/fakes/empty.json
$ python3 -m stepmold eval ticket_triage --model fake:fakes/empty.json
stepmold: /.../ticket_triage/fakes/empty.json is not valid JSON (FakeModel needs at least one scripted response)
```
...
$ python3 -m stepmold eval ticket_triage --model fake:fakes/emptyarr.json
stepmold: /.../ticket_triage/fakes/emptyarr.json is not valid JSON (FakeModel needs at least one scripted response)
```

A script that is neither raises `TypeError`, which `stepmold/cli.py:main` does not catch — so
what reaches the terminal is a full Python traceback, not a `stepmold:` line (22 lines on
CPython 3.14; the frame list varies by version). First two and last, the rest elided:

```
$ printf '"x"' > ticket_triage/fakes/str.json
$ python3 -m stepmold eval ticket_triage --model fake:fakes/str.json
Traceback (most recent call last):
[the frames elided — they run through stepmold/cli.py:_fake_model and stepmold/model.py:__post_init__,
...
(`stepmold/model.py:FakeModel._next_ordered`). Past the end it raises `ModelExhausted`:
...
The rules, all from `stepmold/model.py:FakeModel._keyed`:
...
PY
$ python3 -m stepmold eval stdrift
...
(`stepmold/codegen.py:build_prompt`). The convention `ticket_triage` and
...
you fix an upstream answer), and anything appended by stepmold itself. A retry appends
`"Your previous answer was rejected: ..."` (`stepmold/codegen.py:ERROR_BLOCK`) at the end of
...
stepmold append `"Your notes from thinking this through: ..."` (`SCRATCHPAD_BLOCK`) at the end.
...
| --- | --- | --- |
| A plain `generate` node, first draw accepted | 1 | `stepmold/verify.py:run_node` |
| A rejected draw | +1 per retry rung; `retries` defaults to **2**, so up to 3 | `stepmold/verify.py:run_node` (`rungs = node.retries + 1`) |
| `two_stage: true` | +1 for the think stage, per rung that re-thinks | `stepmold/codegen.py:generate_once` |
| An `assert` node (`type: assert`) | 0 — it evaluates an expression | `stepmold/graph.py`, and `examples/invoice_extract` has five of them |
| A `tool` node (`type: tool`) | 0 — it calls a function the host registered, never the model | `stepmold/tools.py`, and `examples/refund_desk` has two |
| An `end` node | 0 | |
| A node the case never reaches | 0 | branch not taken |
| A prompt naming state nobody wrote | 0 — `MissingVariable` skips the ladder | `stepmold/verify.py:run_node` |

There are exactly four node types: `generate`, `assert`, `tool`, `end`
(`stepmold/pack.py:NODE_TYPES`).
...
| Behaviour | Detail |
| --- | --- |
| The path resolves **inside the pack** | `stepmold/cli.py:_fake_model` calls `stepmold/pack.py:_resolve_inside`. Absolute paths, `..`, and symlinks escaping the root are refused. |
...
| `fake:` needs no `--allow-pack-model` | Only `openai:` chosen by a manifest does. A fake is local and contained, which is what lets a pack ship its own offline model. See `stepmold/cli.py:resolve_model`. |
| `--model` overrides the manifest | `python3 -m stepmold eval ticket_triage --model fake:fakes/ordered.json`, or point at a real backend: `--model openai:http://localhost:8000#qwen3-8b`. |
...
```
$ python3 -m stepmold eval ticket_triage --model fake:/etc/hosts
stepmold: fake: /etc/hosts: absolute paths are not allowed in a pack

$ python3 -m stepmold eval ticket_triage --model fake:../script.json
stepmold: fake: ../script.json: resolves outside the pack directory

$ ln -s /etc/hosts ticket_triage/fakes/link.json
$ python3 -m stepmold eval ticket_triage --model fake:fakes/link.json
stepmold: fake: fakes/link.json: resolves outside the pack directory

$ python3 -m stepmold eval ticket_triage --model fake:fakes/nope.json
stepmold: fake model script not found: /.../ticket_triage/fakes/nope.json
```

### A network manifest cannot be evaluated at all

If a pack's manifest names an `openai:` endpoint rather than a fake, `stepmold eval` refuses
...
$ python3 -m stepmold eval netpack
stepmold: this pack's manifest selects a network endpoint ('openai:http://localhost:8000#qwen3-8b'). Pass --model to choose the endpoint yourself, or --allow-pack-model to accept the pack's choice.
$ echo $?
1

$ python3 -m stepmold eval netpack --allow-pack-model
...
`--allow-pack-model` is registered on `stepmold run` only (`stepmold/cli.py:build_parser`), so under
`eval` the only way past it is `--model`:

```
$ python3 -m stepmold eval netpack --model fake:fakes/script.json
...
`stepmold/cli.py:_fake_model` returns a single `FakeModel`, and `stepmold/eval.py:evaluate` uses it
for every case. Through `stepmold eval`, therefore:
...
| --- | --- |
| `model` may be a zero-argument **factory** | Each case gets a fresh model, so an ordered script can be per-case rather than global (`stepmold/eval.py:_model_for`). |
| `cases=` overrides the pack's evalset | Score a subset, or an `EvalCase` you built in memory. There is no CLI equivalent — `stepmold eval` always runs the whole file. |
...
This is the expensive part of authoring a pack, and stepmold does not help you with it.
...
**Nothing checks this.** `stepmold/pack.py:load_pack` validates the manifest, the graph, the
grammars and every case's `end` — it never opens the fake script. Neither does
`stepmold validate`, which exits 0 even when the script file is gone outright:

```
$ cp -r ticket_triage noscript
$ rm noscript/fakes/script.json
$ python3 -m stepmold validate noscript
ticket_triage v1: 5 nodes, 3 edges, 3 evalset cases, entry 'classify'
$ echo $?
0
$ python3 -m stepmold eval noscript
stepmold: fake model script not found: /.../noscript/fakes/script.json
```
...
| --- | --- |
| `stepmold eval` | a per-case failure. `stepmold/eval.py:_run_case` catches every `Exception`, so the case fails and the rest of the eval continues. |
| `stepmold run` | an uncaught traceback. `ModelExhausted` is a `RuntimeError`, not a `StepmoldError`, so `stepmold/cli.py:main` does not catch it. |
...
$ python3 -m stepmold eval ticket_triage --model fake:fakes/missing.json
...
That message is the recipe, not just the error. The same gap under `stepmold run` is a
traceback, ending:

```
$ python3 -m stepmold run ticket_triage --model fake:fakes/missing.json --input '{"ticket_id": "T-3", "text": "?????"}'
Traceback (most recent call last):
[the frames elided — the last is stepmold/model.py in _keyed, and each Python version formats
 the body differently]
stepmold.model.ModelExhausted: FakeModel has no scripted response matching prompt: 'Task: priority\nTicket: T-3\n\nGiven the topic, say whether this needs a human now (urgent) or not (normal).\n\nTopic: bug\nText: ?????\n'
```
...
2. Run `python3 -m stepmold eval <pack>`. The first missing key names itself in the
   `ModelExhausted` text.
...
stays forever. Add a check that fails on unused keys. This one needs nothing but stepmold:
...
`FakeModel.calls` (`stepmold/model.py:Call`) is the whole reason this works: it records every
...
```
$ python3 -m stepmold eval drift
ticket_triage: 3/3 cases passed

$ python3 -m stepmold eval drift --model fake:fakes/ordered.json
...
$ python3 -m stepmold eval ticket_triage --model fake:fakes/ordered.json
ticket_triage: 3/3 cases passed
```
...
`--log-level` and `--log-format` are on every subcommand (`stepmold/cli.py:_observability_options`);
...
$ python3 -m stepmold run ticket_triage --model fake:fakes/retry.json --log-level info --input '{"ticket_id": "T-1", "text": "I was charged twice for one order"}'
18:02:36.211 INFO  stepmold.graph run.start run_id=82b5b44d9eb24c229060a7711e3c340d pack=ticket_triage version=1 entry=classify resumed=false max_steps=8 inputs=text,ticket_id
18:02:36.211 WARNING stepmold.verify node.rejected node=classify attempt=1 cause=verify reason="output was not valid JSON — return a single JSON object and nothing else" of=3
18:02:36.211 INFO  stepmold.verify node.retry node=classify attempt=2 of=3 temperature=0.5 seed=1 reason="output was not valid JSON — return a single JSON object and nothing else" rethink=false
18:02:36.212 INFO  stepmold.graph node.ok run_id=82b5b44d9eb24c229060a7711e3c340d node=classify type=generate attempts=2 output=merge duration_ms=0.3
18:02:36.212 INFO  stepmold.graph node.ok run_id=82b5b44d9eb24c229060a7711e3c340d node=priority type=generate attempts=1 output=merge duration_ms=0.0
18:02:36.212 INFO  stepmold.graph run.end run_id=82b5b44d9eb24c229060a7711e3c340d pack=ticket_triage end_node=escalated steps=3 generations=3 failures=0 output_keys=2 output_bytes=38 duration_ms=0.7
...
`stepmold/pack.py:_load_evalset` into `stepmold/pack.py:EvalCase`.
...
EOF
$ python3 -m stepmold validate broken
stepmold: pack error: evalset.jsonl:2: missing or non-object 'input'
$ echo $?
1
```
...
EOF
$ python3 -m stepmold validate broken
stepmold: pack error: evalset.jsonl:1: missing or non-object 'expect'
```
...
$ python3 -m stepmold eval broken
...
`end` closes it (`stepmold/eval.py:_compare_ending`). A mismatch is reported under the pseudo
field `<ending>`, as in the routing sabotage further down.

A typo in `end` is caught at **load** time, not silently never-matched
(`stepmold/pack.py:_check_case_endings`): the name must exist in `graph.yaml` and must be a
...
EOF
$ python3 -m stepmold validate broken
stepmold: pack error: evalset.jsonl: case 'typo' expects ending 'esclated', which is not a node in graph.yaml

$ cat > broken/evalset.jsonl <<'EOF'
{"name": "not an ending", "input": {"ticket_id": "T-1", "text": "x"}, "expect": {}, "end": "priority"}
EOF
$ python3 -m stepmold validate broken
stepmold: pack error: evalset.jsonl: case 'not an ending' expects ending 'priority', but that node is type 'generate', not 'end'
```
...
(`stepmold/eval.py:_run_case`), so it cannot be used to silence a real failure:
...
$ python3 -m stepmold eval sabotage
ticket_triage: 2/3 cases passed
...
`stepmold/eval.py:_compare` uses `!=`. There is no substring match, no numeric tolerance, no
...
```
$ python3 -m stepmold eval tagpack
...
`stepmold/eval.py:_actual` looks in the end node's **projected output** first, then the **full
...
from stepmold.cli import resolve_model
from stepmold.eval import evaluate
from stepmold.pack import EvalCase, load_pack
...
not the field listed first in `expect` (`stepmold/eval.py:_earliest_node`) — key order in a
...
case. `--json` (`stepmold/cli.py:_report_json`) is the only way to see it from the CLI. It
carries `pack`, `passed`, `failed`, `total`, `by_node`, and per case `name`, `passed`,
`node`, `error`, `expected`, `actual`, `mismatches`:

```
$ python3 -m stepmold eval ticket_triage --json | python3 -c 'import json,sys
...
## `stepmold run` and `stepmold eval` are different surfaces

`stepmold eval` takes `--model`, `--json` and the two log options — nothing else. `stepmold run`
takes rather more (`stepmold/cli.py:build_parser`), and none of that applies under `eval`:
...
```
$ python3 -m stepmold run ticket_triage --input '{"ticket_id": "T-1", "text": "I was charged twice for one order"}' --state
{"text": "I was charged twice for one order", "ticket_id": "T-1", "tier": "urgent", "topic": "billing"}

$ python3 -m stepmold run ticket_triage --resume demo-1
...
| --- | --- |
| 0 | every case passed |
| 1 | any case failed; or a `PackError` / `StepmoldError` / `ValueError` — including "no model", a fake script outside the pack, an unreadable pack; or an uncaught traceback such as `ModelExhausted` |
| 2 | argparse rejected the command line (`stepmold eval` with no pack argument, `--allow-pack-model` under `eval`) |

An **empty evalset is not a pass**. `stepmold/eval.py:evaluate` raises, and the CLI turns it
into exit 1 — but note that `stepmold validate` is happy with it, so this is a gate `eval`
alone holds:

```
$ : > broken/evalset.jsonl
$ python3 -m stepmold eval broken
stepmold: pack 'ticket_triage' has no evalset cases to run — an empty evalset is not a pass
$ echo $?
1
$ python3 -m stepmold validate broken
