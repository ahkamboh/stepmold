# jig — the agent compiler for small models

> **A machinist's jig is a custom guide that lets a cheap tool do precision work, repeatably, forever.**
> jig is that for LLMs: the big model builds the jig once; a small model runs inside it thousands of times.

- Working name: **`jig`** (npm-safe fallback: `agentjig`)
...
jig is the third path: **don't make the model smarter, make the task easier.** All thinking moves to compile time; the small model only ever does one short, constrained, verified step.
...
2. **"We can't send this data anywhere"** (banks, health, legal, gov, defense) — today their options are *no AI* or *a GPU cluster for a 70B*. jig makes an 8B on one modest GPU reliable enough to use. **Not a discount — access they didn't have.** Strongest wedge.
...
5. **Vendor lock-in** — model swap = `jig build --model X` gated on the evalset, not a rewrite.
6. **Offline/edge** — static binary + pack + local model, no internet.

<!-- a section on commercial strategy is kept out of the public repo -->
### What jig does NOT solve
Open-ended novel tasks (plan must be invented per request); workflows run ~10×/month (build never amortizes — just call an API); bad requirements (50 bad examples compile a bad jig); research/exploration.
**Boundary: jig is for workflows repeated enough that "compile once" pays off. Repetition is the qualifier.**
...
jig is a **compiler**. It splits an agent's life into two phases:

| Phase | Model | Frequency | Cost |
|---|---|---|---|
| **jig build** (compile) | Frontier (Opus-class) | Once per workflow + on regression | ~$5–50, one-time |
| **jig run** (execute) | Small (4B–14B, self-hosted) | Every request | ~$0.15/M tokens |

The compile step produces a **JigPack** — a versioned, cached bundle of five artifacts. The runtime never improvises; it executes the pack.

```
jig build  ──►  JigPack v3
...
jig run    ──►  small model + JigPack on one shared vLLM/SGLang instance
```

**The eval set is the source of truth, not the prompts.** Prompts, grammars, and graphs are all regenerable build outputs. The ~50 gold examples per workflow are the only hand-maintained asset. Model changed? `jig build` again. This is CI/CD for agents.
...
**Fix (in jig):**
- Backend on **XGrammar-2**: <40µs/token overhead (bitwise mask ops), grammar automaton compiled once in 20–50ms, then cached. Per-request overhead on repeated schemas drops **to ~zero after warmup** — and jig's whole model is repeated schemas.
- **Compile at build time, not request time**: `jig build` runs the grammar compiler over every node schema and ships the compiled automata inside the JigPack. Runtime never compiles anything.
...
**Fix (in jig):** **two-stage generation is the compiler's default codegen**, not an option the developer must remember:
...
The compiler auto-inserts stage 1 only on nodes whose eval-set error rate justifies it (measured during `jig build`), so trivial nodes stay single-stage and cheap. Alternative codegen for simple nodes: `reasoning` field ordered before `answer` in the schema.
...
**Fix (in jig):**
- Optimization runs **inside `jig build`**, per node, against the evalset — treated exactly like a compiler optimization pass (GEPA as the default optimizer; the Shopify result — GPT-5 task → small Qwen, ~75× cheaper, ~2× more reliable — is the proof this pass pays for itself).
- **Per-node signal, not end-to-end**: because the graph gives every node its own input/output contract, jig can localize which node fails — solving DSPy's "black box end-to-end only" limitation structurally.
- **Model swap = rebuild, not rewrite.** `jig build --model qwen3-8b` recompiles all prompts against the new model and gates on the evalset. Build artifacts are content-hashed and cached: only nodes whose (schema, evalset slice, model) changed get re-optimized. Incremental compilation, like any real compiler.
...
**Fix (in jig):** three execution tiers, chosen per node at compile time:
- **Tier 0 — whitelisted namespace (default, ~90% of nodes):** the node can only call functions declared in the JigPack manifest; runs in-process, zero isolation cost. Most "agentic" client work is tool orchestration, not arbitrary code.
...
- The **environment snapshot ships in the JigPack** (`snapshot.fc`): deps pre-installed, tools pre-imported — so tier-1 restore lands in a ready interpreter, not a booting one.
...
- Not RAG, not fine-tuning (v1 is strictly inference-time; ART/RULER-style RL is a far-future `jig train` if ever)
- Not for open-ended novel tasks — jig targets **defined, repeatable workflows**, which is exactly where the money is being burned today
...
2. **M1 — `jig build` (2–3 wk):** evalset format, GEPA pass per node, content-hashed incremental artifact cache, `jig build --model` swap gate.
3. **M2 — graph induction (2 wk):** frontier model proposes graph from task+examples, human-approved diff, evalset gate.
4. **M3 — scale layer (3–4 wk):** shared serving with prefix-ordered prompts, Firecracker warm pool (tier 1), per-node tracing/replay, `jig run --serve` multi-pack.
5. **M4 — polish:** JigPack registry, OSS release (MIT, same playbook as scrolltape/agent-eyes), docs with the cost benchmark as the headline.
...
**Python 3.12** for the whole framework. Rationale: jig is orchestration glue — all performance-critical work already lives in C++/CUDA/Rust engines we call, and every mandatory dependency is Python-first (DSPy/GEPA has no non-Python equivalent; XGrammar's primary API is its Python binding). Runtime is I/O-bound on GPU inference → asyncio scales it.

| Component | Choice |
|---|---|
| `jig build` compiler | Python 3.12 (DSPy/GEPA, xgrammar bindings, content-hash cache) |
| `jig run` executor | Python + asyncio (graph walker, retry ladder) → SGLang over HTTP |
| CLI | Typer + Rich |
| JigPack format | **Language-agnostic**: JSON/YAML manifest + artifacts (enables future TS runtime SDK) |
...
| Packaging | `uv` + pyproject, PyPI as `jig` (fallback `agentjig`) |
...
jig has two halves with **opposite** requirements — that's why there's no single answer:

| | `jig build` (compiler) | `jig run` (runtime) |
|---|---|---|
...
The language-agnostic JigPack format is what makes a non-Python runtime legal later without touching the compiler. Deliberate.
...
**Business driver (the operator on-prem):** shipping `jig run` as one static binary (`scp` it) vs. a Python env (3.12 + uv + ~40 deps + CUDA version matching) across 30 client boxes is the difference between a product and a support burden. This — not throughput — is the real reason to port.

**Sequenced call:**
1. M0–M2: **Python for everything.** Do not split while the runtime spec is still moving.
2. M3: port `jig run` to **Go** if on-prem deploy pain is real; keep Python runtime as reference impl + dev mode.
3. `jig build` stays Python permanently.
...
Note for grammar backend choice: llguidance (Rust, ~50µs/tok) beats XGrammar on **unique/dynamic** schemas; XGrammar wins on **repeated** schemas via caching. jig is repeated-schema by architecture → **XGrammar-2 stays the pick**.

### 7.2 Size / compile-speed constraints (stated priority)

**Key insight: the compiler never ships.** gcc doesn't ship with the binary it compiled; `jig-build` doesn't ship with the agent it compiled. Python's weight is a build-time cost on hardware we control. The client receives one static binary + one pack.

```
BUILD MACHINE (ours)                    CLIENT SERVER
jig-build (Python, DSPy/GEPA,  ──pack──► jig (Go static, <10MB)
xgrammar, 500MB+ deps)                   + JigPack (<500KB)
                                         no Python, no CUDA deps
```

| Artifact | Target | Method |
|---|---|---|
| `jig` runtime binary | **<10MB** | Go, `CGO_ENABLED=0`, `-ldflags="-s -w"` |
| JigPack | **<500KB** | all text: prompts, JSON grammars, graph |
| Cold start | **<20ms** | pack parsed once at boot |
| Full compile | **<3s** | dependency budget below |

Build: `CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o jig ./cmd/jig`
...
**Where "fast" actually comes from.** jig's own CPU work is microseconds; each node then waits ~200ms on the GPU — so language speed is irrelevant to end-to-end latency. The real levers, in order:
...
- **Evalset quality is the ceiling** — 50 bad examples compile a bad jig. Mitigation: evalset linter + frontier-model review pass during build.
- **XGrammar/SGLang API churn** — pin versions in the pack; artifacts declare their backend version.
- **"Compile step" adoption friction** — devs expect `pip install && run`. Mitigation: `jig init` does M2's induction interactively so the first build feels like magic, not homework.
- **Name collision check before publish** — `jig` is short; verify npm/PyPI/GitHub availability, fall back to `agentjig`.
