...
`jig/graph.py`'s own words, "the one thing in a run that retrying cannot undo" — so the
...
deliberately no manifest key for this (`jig/cli.py:_add_tools_option`): a pack you did not
...
```
$ python3 -m jig eval examples/refund_desk --tools examples/refund_desk/tools.py --tiers
refund_desk: 12/12 cases passed
...
```
$ python3 -m jig run examples/refund_desk --tools examples/refund_desk/tools.py \
...
`jig.pack._NODE_KEYS` does not carry them and `jig.pack.Node` has no field for them, so the
...
p.write_text(p.read_text().replace(
    ...
    '    samples: 3\n    agree: 2\n    on_unsure: needs_human'))
"
$ python3 -m jig validate /tmp/rd-samples
jig: pack error: graph.yaml: node 'approve' has unknown key(s): agree, samples
```

The gate itself is real and shipped: `jig.verify.gate_for` reads the two keys off the node
with `getattr`, `run_node` draws and compares, and `jig.graph` routes `Unsure` to
...
AGREED  (draw 1 == draw 2)
  ...
  log         WARNING jig.verify node.samples.blind node=approve samples=3 model=FakeModel reason="backend takes no sampling hint, so extra draws repeat the first"
  log         INFO  jig.verify node.agreed node=approve agreed=2 of=2 required=2 asked=3 generations=2
...
UNSURE  (a real disagreement)
  ...
  log         WARNING jig.verify node.samples.blind node=approve samples=3 model=FakeModel reason="backend takes no sampling hint, so extra draws repeat the first"
  log         WARNING jig.verify node.unsure node=approve agreed=1 of=3 required=2 asked=3 distinct=3 generations=3
  log         INFO  jig.graph edge.on_unsure run_id=d6b459e49e7e4565964f3ab648474dc9 node=approve to=needs_human
...
UNSURE  (same decision, three wordings)
  ...
  log         WARNING jig.verify node.samples.blind node=approve samples=3 model=FakeModel reason="backend takes no sampling hint, so extra draws repeat the first"
  log         WARNING jig.verify node.unsure node=approve agreed=1 of=3 required=2 asked=3 distinct=3 generations=3
  log         INFO  jig.graph edge.on_unsure run_id=532fed4bba0642efa98b7c0da5e08c22 node=approve to=needs_human
  path        classify -> lookup -> assess -> approve -> needs_human
  end node    needs_human
  ledger      []
  refunded?   False
...
`sampling` keyword, so `codegen.accepts_sampling` is false and jig warns that the extra
...
```
$ python3 -m jig eval examples/refund_desk
...
**Scoring this pack moves money.** `jig eval` is not a dry run: five of the twelve cases
...
jig's exactly-once record stopped working, instead of a quiet second row in the ledger.
...
`jig run` answer its own case instead of the first one. The footer means nothing to a real
